...
mkdir -p "$APP_DIR/usr/lib/qt6/plugins/platforms" && cp /usr/lib/qt6/plugins/platforms/libqxcb.so "$APP_DIR/usr/lib/qt6/plugins/platforms/"
...
if [ ! -f NO_STRIP=1 "$LINUXDEPLOY" ]; then
    echo "--> Downloading linuxdeploy..."
    wget -q -O NO_STRIP=1 "$LINUXDEPLOY" "https://github.com/linuxdeploy/linuxdeploy/releases/download/continuous/linuxdeploy-x86_64.AppImage"
    chmod +x NO_STRIP=1 "$LINUXDEPLOY"
fi
