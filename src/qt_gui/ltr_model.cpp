...
void ModelCreate::on_CancelButton_pressed() { reject(); }
...
void ModelCreate::on_CreateButton_pressed() {
  ...
  if (PREF.createSection(sec)) {
    if (ui.ModelTypeCombo->currentIndex() == 3) {
      PREF.addKeyVal(sec, QString::fromUtf8("Model-type"),
                     QString::fromUtf8("Face"));
    } else if (ui.ModelTypeCombo->currentIndex() == 4) {
      PREF.addKeyVal(sec, QString::fromUtf8("Model-type"),
                     QString::fromUtf8("Absolute"));
    } else {
      emit dump(sec);
    }
  }
...
ModelEdit::ModelEdit(Guardian *grd, QWidget *parent)
    : QWidget(parent), modelTweaker(0), initializing(false) {
  grd->regTgt(this);
  ui.setupUi(this);
...
void ModelEdit::on_CreateModelButton_pressed() { mcw->exec(); }
...
void ModelEdit::modelSelectorActivated(const QString &text) {
  ...
  if (type.compare(QString::fromUtf8("Cap"), Qt::CaseInsensitive) == 0) {
    ...
    if (text == QString::fromUtf8("NP TrackClip")) {
      ui.ModelPreview->setPixmap(
          QPixmap(QString::fromUtf8(":/ltr/cap_np.png")));
    } else {
  ...
  } else if (type.compare(QString::fromUtf8("Clip"), Qt::CaseInsensitive) ==
             0) {
    // ui.ModelTypeLabel->setText("3 Point Clip");
    if (text == QString::fromUtf8("NP TrackClip Pro")) {
      ui.ModelPreview->setPixmap(
          QPixmap(QString::fromUtf8(":/ltr/clip_np.png")));
    } else {
      ui.ModelPreview->setPixmap(
          QPixmap(QString::fromUtf8(":/ltr/clip_1.png")));
    }
    modelTweaker = new ClipTweaking(currentSection, this);
    modelType = MDL_3PT_CLIP;
  } else if (type.compare(QString::fromUtf8("Face"), Qt::CaseInsensitive) ==
             0) {
...
CapEdit::CapEdit(QWidget *parent) : QWidget(parent) { ui.setupUi(this); }

CapEdit::~CapEdit() {}

void CapEdit::dump(const QString &sec) {
  PREF.addKeyVal(sec, QString::fromUtf8("Model-type"),
                 QString::fromUtf8("Cap"));
  PREF.addKeyVal(sec, QString::fromUtf8("Cap-Y"),
                 QString::number(ui.CapA->value()));
  PREF.addKeyVal(sec, QString::fromUtf8("Cap-X"),
                 QString::number(ui.CapB->value()));
  PREF.addKeyVal(sec, QString::fromUtf8("Cap-Z"),
                 QString::number(ui.CapC->value()));
...
ClipEdit::ClipEdit(QWidget *parent) : QWidget(parent) { ui.setupUi(this); }

ClipEdit::~ClipEdit() {}

void ClipEdit::dump(const QString &sec) {
  PREF.addKeyVal(sec, QString::fromUtf8("Model-type"),
                 QString::fromUtf8("Clip"));
  PREF.addKeyVal(sec, QString::fromUtf8("Clip-Y1"),
                 QString::number(ui.ClipB->value()));
  PREF.addKeyVal(sec, QString::fromUtf8("Clip-Y2"),
                 QString::number(ui.ClipB->value() + ui.ClipC->value()));
  PREF.addKeyVal(sec, QString::fromUtf8("Clip-Z1"),
                 QString::number(ui.ClipA->value()));
...
SingleEdit::SingleEdit(QWidget *parent) : QWidget(parent) { ui.setupUi(this); }

SingleEdit::~SingleEdit() {}

void SingleEdit::dump(const QString &sec) {
  PREF.addKeyVal(sec, QString::fromUtf8("Model-type"),
                 QString::fromUtf8("SinglePoint"));
  QString v;
...
CapTweaking::CapTweaking(const QString &section, QWidget *parent)
    : QWidget(parent), currentSection(section), initializing(true) {
  ui.setupUi(this);
  QString val;
...
ClipTweaking::ClipTweaking(const QString &section, QWidget *parent)
    : QWidget(parent), currentSection(section), initializing(true) {
  ui.setupUi(this);
  QString val;
...
void ClipTweaking::tweakHx() {
  if (!initializing) {
    ...
    PREF.setKeyVal(currentSection, QString::fromUtf8("Head-X"),
                   ui.ClipHx->value() * sign);
  }
  PREF.announceModelChange();
}
...
void ClipTweaking::on_ClipLeft_toggled() { tweakHx(); }

void ClipTweaking::on_ClipRight_toggled() { tweakHx(); }
