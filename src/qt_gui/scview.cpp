...
SCView::SCView(axis_t a, QWidget *parent)
    : QWidget(parent), parentWidget(parent), px(0.0), axis(a), timer(nullptr),
      invert(false) {
  ...
  connect(&TRACKER,
          SIGNAL(newPose(linuxtrack_full_pose_t *, linuxtrack_pose_t *,
                         linuxtrack_pose_t *)),
          this,
          SLOT(newPose(linuxtrack_full_pose_t *, linuxtrack_pose_t *,
                       linuxtrack_pose_t *)));
  timer->start(50);
...
void SCView::redraw() { update(); }

// QSize SCView::sizeHint() const
//{
//   if(parentWidget){
//     std::cout<<"Size: "<<parentWidget->size().height() <<" "<<
...
int SCView::spline(QPointF points[], int num_points) {
  ...
  kl = (TRACKER.axisGet(axis, AXIS_MULT) != 0.0)
           ? TRACKER.axisGet(axis, AXIS_LLIMIT) /
                 TRACKER.axisGet(axis, AXIS_MULT)
           : 0.0;
  kr = (TRACKER.axisGet(axis, AXIS_MULT) != 0.0)
           ? TRACKER.axisGet(axis, AXIS_RLIMIT) /
                 TRACKER.axisGet(axis, AXIS_MULT)
           : 0.0;
  max_k = kl > kr ? kl : kr;
...
float SCView::spline(float x) { return fabs(TRACKER.axisGetValue(axis, x)); }
...
void SCView::paintEvent(QPaintEvent * /* event */) {
  std::cerr << "SCView: paintEvent start for axis " << axis << " in thread "
...
void SCView::newPose(linuxtrack_full_pose_t *raw_pose,
                     linuxtrack_pose_t *unfiltered, linuxtrack_pose_t *pose) {
  (void)pose;
  switch (axis) {
  case PITCH:
