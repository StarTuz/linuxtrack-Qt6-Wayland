...
ProfileSetup::~ProfileSetup() { delete sc; }

void ProfileSetup::connect() {
  QObject::connect(&TRACKER, SIGNAL(initAxes(void)), this,
                   SLOT(initAxes(void)));
  QObject::connect(&TRACKER, SIGNAL(axisChanged(int, int)), this,
                   SLOT(axisChanged(int, int)));
  QObject::connect(&TRACKER, SIGNAL(setCommonFF(float)), this,
                   SLOT(setCommonFF(float)));
}
...
void ProfileSetup::on_DetailedAxisSetup_pressed() { sc->show(); }

void ProfileSetup::copyFromDefault() {
  if (QMessageBox::warning(
          this, QString::fromUtf8("Warning:"),
          QString::fromUtf8("Are you sure, you want to overwrite the current "
                            "profile by default values?"),
...
void ProfileSetup::initAxes() {
  ui.PitchEnable->setCheckState(
      TRACKER.axisGetBool(PITCH, AXIS_ENABLED) ? Qt::Checked : Qt::Unchecked);
  ui.YawEnable->setCheckState(
      TRACKER.axisGetBool(YAW, AXIS_ENABLED) ? Qt::Checked : Qt::Unchecked);
  ui.RollEnable->setCheckState(
      TRACKER.axisGetBool(ROLL, AXIS_ENABLED) ? Qt::Checked : Qt::Unchecked);
  ui.TxEnable->setCheckState(
      TRACKER.axisGetBool(TX, AXIS_ENABLED) ? Qt::Checked : Qt::Unchecked);
  ui.TyEnable->setCheckState(
      TRACKER.axisGetBool(TY, AXIS_ENABLED) ? Qt::Checked : Qt::Unchecked);
  ui.TzEnable->setCheckState(
      TRACKER.axisGetBool(TZ, AXIS_ENABLED) ? Qt::Checked : Qt::Unchecked);
  ui.PitchInvert->setCheckState(
      TRACKER.axisGetBool(PITCH, AXIS_INVERTED) ? Qt::Checked : Qt::Unchecked);
  ui.YawInvert->setCheckState(
      TRACKER.axisGetBool(YAW, AXIS_INVERTED) ? Qt::Checked : Qt::Unchecked);
  ui.RollInvert->setCheckState(
      TRACKER.axisGetBool(ROLL, AXIS_INVERTED) ? Qt::Checked : Qt::Unchecked);
  ui.TxInvert->setCheckState(
      TRACKER.axisGetBool(TX, AXIS_INVERTED) ? Qt::Checked : Qt::Unchecked);
  ui.TyInvert->setCheckState(
      TRACKER.axisGetBool(TY, AXIS_INVERTED) ? Qt::Checked : Qt::Unchecked);
  ui.TzInvert->setCheckState(
      TRACKER.axisGetBool(TZ, AXIS_INVERTED) ? Qt::Checked : Qt::Unchecked);
...
void ProfileSetup::on_PitchEnable_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(PITCH, AXIS_ENABLED, (state == Qt::Checked));
}

void ProfileSetup::on_YawEnable_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(YAW, AXIS_ENABLED, (state == Qt::Checked));
}

void ProfileSetup::on_RollEnable_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(ROLL, AXIS_ENABLED, (state == Qt::Checked));
}

void ProfileSetup::on_TxEnable_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(TX, AXIS_ENABLED, (state == Qt::Checked));
}

void ProfileSetup::on_TyEnable_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(TY, AXIS_ENABLED, (state == Qt::Checked));
}

void ProfileSetup::on_TzEnable_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(TZ, AXIS_ENABLED, (state == Qt::Checked));
}

void ProfileSetup::on_PitchInvert_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(PITCH, AXIS_INVERTED, (state == Qt::Checked));
}

void ProfileSetup::on_YawInvert_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(YAW, AXIS_INVERTED, (state == Qt::Checked));
}

void ProfileSetup::on_RollInvert_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(ROLL, AXIS_INVERTED, (state == Qt::Checked));
}

void ProfileSetup::on_TxInvert_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(TX, AXIS_INVERTED, (state == Qt::Checked));
}

void ProfileSetup::on_TyInvert_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(TY, AXIS_INVERTED, (state == Qt::Checked));
}

void ProfileSetup::on_TzInvert_stateChanged(int state) {
  if (!initializing)
    TRACKER.axisChange(TZ, AXIS_INVERTED, (state == Qt::Checked));
}

void ProfileSetup::on_PitchSens_valueChanged(int val) {
  if (!initializing)
    TRACKER.axisChange(PITCH, AXIS_MULT, val / 12.0f);
}

void ProfileSetup::on_YawSens_valueChanged(int val) {
  if (!initializing)
    TRACKER.axisChange(YAW, AXIS_MULT, val / 12.0f);
}

void ProfileSetup::on_RollSens_valueChanged(int val) {
  if (!initializing)
    TRACKER.axisChange(ROLL, AXIS_MULT, val / 12.0f);
}

void ProfileSetup::on_TxSens_valueChanged(int val) {
  if (!initializing)
    TRACKER.axisChange(TX, AXIS_MULT, val / 12.0f);
}

void ProfileSetup::on_TySens_valueChanged(int val) {
  if (!initializing)
    TRACKER.axisChange(TY, AXIS_MULT, val / 12.0f);
}

void ProfileSetup::on_TzSens_valueChanged(int val) {
  if (!initializing)
    TRACKER.axisChange(TZ, AXIS_MULT, val / 12.0f);
}
...
void ProfileSetup::exportProfile(QTextStream &tf) {
  ...
  for (int i = PITCH; i <= TZ; ++i) {
    tf << (TRACKER.axisGetBool((axis_t)i, AXIS_ENABLED)
               ? QString::fromUtf8("1")
               : QString::fromUtf8("0"))
       << QString::fromUtf8(" ");
    tf << (TRACKER.axisGetBool((axis_t)i, AXIS_INVERTED)
               ? QString::fromUtf8("1")
               : QString::fromUtf8("0"))
       << QString::fromUtf8(" ");
    for (int j = AXIS_DEADZONE; j < AXIS_FILTER; ++j) {
