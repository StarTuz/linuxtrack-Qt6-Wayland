...
static bool start_master() {
  ...
  if (socket < 0) {
    ...
    if (!getenv("LTR_KEEP_LD_LIBRARY_PATH")) {
      ...
      ltr_int_log_message(
          "LTR_KEEP_LD_LIBRARY_PATH set, keeping LD_LIBRARY_PATH set.\n");
    }
    ...
    wait(&status);
    // At this point master is either running or exited (depending on the state
    // of socket)
    free(args[0]);
  }
  close(socket);
...
static bool open_master_comms(int *l_master_uplink) {
  ...
  *l_master_uplink = ltr_int_connect_to_socket(ltr_int_master_socket_name());
  ...
  if (ltr_int_send_message_w_str(*l_master_uplink, CMD_NEW_SOCKET, 0,
                                 profile_name) != 0) {
...
static bool ltr_int_process_message(int l_master_uplink) {
  ...
  ssize_t bytesRead =
      ltr_int_socket_receive(l_master_uplink, &msg, sizeof(message_t));
  if (bytesRead < 0) {
...
static void *ltr_int_slave_reader_thread(void *param) {
  ...
  while (1) {
    ...
    struct pollfd uplink_poll = {
        .fd = master_uplink, .events = POLLIN, .revents = 0};
    while (1) {
...
bool ltr_int_slave(const char *c_profile, const char *c_com_file,
                   const char *ppid_str, const char *close_pipe_str,
                   const char *notify_pipe_str) {
  ...
  quit_flag = false;
  if (pthread_create(&reader_tid, NULL, ltr_int_slave_reader_thread, NULL) ==
      0) {
