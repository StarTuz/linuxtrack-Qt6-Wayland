...
const char *ltr_int_get_logfile_name(void) { return logfile_name; }
...
void ltr_int_check_root() {
  if (geteuid() == 0) {
    ltr_int_log_message("Linuxtrack components aren't supposed running with "
                        "root privileges!\n");
    printf("Linuxtrack components aren't supposed running with root "
           "privileges!\n");
    exit(0);
  }
}
