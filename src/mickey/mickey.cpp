...
void MickeyApplyDialog::trySettings() {
  ...
  cntr = 15;
  ui.FBString->setText(
      QString::fromUtf8("Will automatically revert back in %1 seconds...")
          .arg(cntr));
  show();
  raise();
  activateWindow();
  // std::cout<<"trying settings!\n";
}

void MickeyApplyDialog::timeout() {
  // std::cout<<"Apl timeout!\n";
  --cntr;
  if (cntr == 0) {
    timer.stop();
    emit revert();
...
MickeyCalibration::MickeyCalibration() {
  ui.setupUi(this);
  timer.setSingleShot(false);
  QObject::connect(ui.CancelButton, SIGNAL(pressed()), this,
                   SLOT(cancelPressed()));
  QObject::connect(&timer, SIGNAL(timeout()), this, SLOT(timeout()));
  setModal(true);
}
...
void MickeyCalibration::recenter() {
  ...
  cntr = GUI.getCntrDelay() +
         1; // +1 - compensation for first increment in timeout()
  timeout();
  window()->show();
  window()->raise();
  window()->activateWindow();
...
void MickeyCalibration::timeout() {
  ...
  case CENTER:
  case CENTER_ONLY:
    ui.CalibrationText->setText(
        QString::fromUtf8("Please center your head and sight."));
    ui.FBString->setText(
        QString::fromUtf8("Center position will be recorded in %1 seconds...")
            .arg(cntr));
    break;
  case CALIBRATE:
    ui.CalibrationText->setText(QString::fromUtf8(
        "Please move your head to left/right and up/down extremes."));
    ui.FBString->setText(
        QString::fromUtf8("Calibration will end in %1 seconds...").arg(cntr));
    break;
  }
}
...
MickeyThread::MickeyThread(Mickey *p)
    : QThread(p), fifo(-1), finish(false), parent(*p), fakeBtn(0) {}

void MickeyThread::processClick(sn4_btn_event_t ev) {
  // std::cout<<"Processing click! ("<<(int)ev.btns<<")\n";
  int btns = ev.btns;
...
#endif
{
  ...
  // SIGNAL(activated()), &btnThread, SLOT(on_key_pressed()));
  QObject::connect(this, SIGNAL(mouseHotKey_activated(int, bool)), &btnThread,
                   SLOT(on_mouseHotKey_activated(int, bool)));
  ...
  QObject::connect(&calDlg, SIGNAL(recenterNow(bool)), this,
                   SLOT(recenterNow(bool)));
  QObject::connect(&calDlg, SIGNAL(startCalibration()), this,
                   SLOT(startCalibration()));
  QObject::connect(&calDlg, SIGNAL(finishCalibration()), this,
                   SLOT(finishCalibration()));
  QObject::connect(&calDlg, SIGNAL(cancelCalibration(bool)), this,
                   SLOT(cancelCalibration(bool)));
...
void Mickey::revertSettings() { trans->revertSettings(); }

void Mickey::keepSettings() { trans->keepSettings(); }
...
void Mickey::hotKey_activated(int id, bool pressed) {
  ...
  case 0: // toggle
    ...
    // std::cout<<"On/off switch activated!\n";
    switch (state) {
    case TRACKING:
      changeState(STANDBY);
      break;
...
void Mickey::updateTimer_activated() {
  ...
  //  if(linuxtrack_get_pose(&heading, &pitch, &roll, &tx, &ty, &tz, &counter) >
  //  0){
  ...
  if (state == TRACKING) {
    if (relative) {
      ...
      QPoint c =
          screenCenter + QPoint(screenCenter.x() * dx, screenCenter.y() * dy);
      QCursor::setPos(c);
    }
  }
}

void Mickey::startCalibration() { trans->startCalibration(); }
...
static HotKey *addHotKey(const QString &label, const QString &prefId, int id,
                         QWidget *owner, QObject *target, QGridLayout *dest,
                         QSettings *pref, int row, int column) {
  ...
  if (hk->setHotKey(hkString)) {
    ...
    QObject::connect(hk, SIGNAL(activated(int, bool)), target,
                     SLOT(hotKey_activated(int, bool)));
    QObject::connect(hk, SIGNAL(newHotKey(const QString &, const QString &)),
                     owner,
                     SLOT(updateHotKey(const QString &, const QString &)));
    return hk;
  } else {
    delete hk;
    return nullptr;
...
void MickeyGUI::updateHotKey(const QString &prefId, const QString &hk) {
  // std::cout<<"Updating Id: "<<prefId.toUtf8().constData()<<"
  // "<<hk.toUtf8().constData()<<"\n";
...
void MickeyGUI::storePrefs() {
  ...
  if (QMessageBox::question(
          this, QString::fromUtf8("Save prefs?"),
          QString::fromUtf8(
...
void MickeyGUI::show() {
  QWidget::show();
  setWindowTitle(QString::fromUtf8("Mickey v") +
...
void MickeyGUI::init() {
  ...
  ui.HotkeyStack->addWidget(new QLabel(QString::fromUtf8("Hotkey setup")), 1,
                            1);
  toggleHotKey = addHotKey(QString::fromUtf8("Start/Stop tracking:"),
                           QString::fromUtf8("tracking_toggle"), 0, this,
                           mickey, ui.HotkeyStack, &settings, 2, 1);
  recenterHotKey = addHotKey(QString::fromUtf8("Quick recenter:"),
                             QString::fromUtf8("quick_recenter"), 3, this,
                             mickey, ui.HotkeyStack, &settings, 2, 2);
  ui.HotkeyStack->addWidget(
      new QLabel(QString::fromUtf8("Mouse button emulation")), 3, 1);
  lmbHotKey =
      addHotKey(QString::fromUtf8("Left:"), QString::fromUtf8("l_mouse"), 1,
                this, mickey, ui.HotkeyStack, &settings, 4, 1);
  mmbHotKey =
      addHotKey(QString::fromUtf8("Middle:"), QString::fromUtf8("m_mouse"), 2,
                this, mickey, ui.HotkeyStack, &settings, 4, 2);
  settings.endGroup();
