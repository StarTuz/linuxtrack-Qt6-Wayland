...
static int my_x_errhandler(Display *display, XErrorEvent *event) {
  if ((event->request_code == X_GrabKey) ||
    ...
    // to be 100% sure there is a place for ending NULL, I sub 1 from
    // ERR_MSG_SIZE
...
static bool grabKeyX(Display *display, Window &window, KeyCode code,
  ...
  // No modifiers
  XGrabKey(display, code, modifiers, window, true, GrabModeAsync,
           GrabModeAsync);
  XSync(display, false);
  if (errorEncountered) {
    // nothing to clean up...
  ...
  // Numlock
  XGrabKey(display, code, modifiers | Mod2Mask, window, true, GrabModeAsync,
           GrabModeAsync);
  XSync(display, false);
  if (errorEncountered) {
  ...
  // ScrollLock
  XGrabKey(display, code, modifiers | Mod5Mask, window, true, GrabModeAsync,
           GrabModeAsync);
  XSync(display, false);
  if (errorEncountered) {
  ...
  // NumLock + ScrollLock
  XGrabKey(display, code, modifiers | Mod2Mask | Mod5Mask, window, true,
