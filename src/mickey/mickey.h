...
class MickeyDialog: public QDialog
{
 ...
 signals:
  void closing();
 protected:
  virtual void closeEvent(QCloseEvent *event){emit closing();
...
public:
  ...
  // Transform interface
  void getMaxVal(float &x, float &y) {
    x = maxValX;
    y = maxValY;
  };
  void setMaxVal(float x, float y) {
    changed = true;
    maxValX = x;
    maxValY = y;
  };
...
private slots:
  ...
  void on_RelativeCB_clicked(bool checked) {
    mickey->setRelative(checked);
    changed = true;
  };
  void on_AbsoluteCB_clicked(bool checked) {
    mickey->setRelative(!checked);
    changed = true;
  };
