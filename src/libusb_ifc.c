...
static bool is_device(libusb_device *dev, unsigned int vid,
                      unsigned int devid) {
...
dev_found ltr_int_find_tir(void) {
  ...
  if (found) {
    ...
    if (err) {
      ...
      ltr_int_log_message(
          "First open attempt failed (%s), retrying after 200ms...\n",
          libusb_strerror(err));
      usleep(200000);
      err = libusb_open(found, &handle);
    }
    if (err) {
      ...
      if (err == LIBUSB_ERROR_ACCESS) {
        ltr_int_log_message("Permission denied - check udev rules.\n");
        return dev | NOT_PERMITTED;
      }
      // Other errors (busy, no_device, etc.) - device found but can't open for
      // non-permission reasons
      ltr_int_log_message(
          "Device found but could not be opened (not a permission issue).\n");
      ltr_int_log_message("Try unplugging and replugging the device.\n");
      return NOT_TIR;
    }
    ltr_int_log_message("Handle opened successfully.\n");
  } else {
...
static bool ltr_int_log_packet(const char *dir, unsigned char data[],
                               size_t size) {
...
bool ltr_int_send_data(int out_ep, unsigned char data[], size_t size) {
  ...
  if ((res = libusb_bulk_transfer(handle, out_ep, data, size, &transferred,
                                  500))) {
    ltr_int_log_message(
        "Problem writing data to TIR@ep %d! %d - %d transferred\n", out_ep, res,
        transferred);
    return false;
  }
...
bool ltr_int_ctrl_data(uint8_t req_type, uint8_t req, uint16_t val,
  ...
  if (comm_dbg_flag == DBG_ON) {
    ltr_int_log_message(
        "***libusb_dump*** req_type: %d req: %d val: %d index: %d\n", req_type,
        req, val, index);
    ltr_int_log_packet("ctrl", data, size);
  }
  // ltr_int_log_message("Sending control data.\n");
  if ((res = libusb_control_transfer(handle, req_type, req, val, index, data,
                                     size, 500)) < 0) {
    ltr_int_log_message("Problem sending control request req_type: %d req: %d "
                        "val: %d index: %d! %d\n",
...
bool ltr_int_receive_data(int in_ep, unsigned char data[], size_t size,
  ...
  *transferred = 0;
  ...
  if ((res = libusb_bulk_transfer(handle, in_ep, data, size, (int *)transferred,
                                  timeout))) {
    if (res != LIBUSB_ERROR_TIMEOUT) {
      ltr_int_log_message(
          "Problem reading data from TIR@ep %d! %d - %d transferred\n", in_ep,
          res, *transferred);
      return false;
    } else {
...
bool ltr_int_reset_usb_device(void) {
  ...
  if (res == 0) {
    ...
    ltr_int_log_message(
        "USB device reset: device re-enumerated (handle may be invalid).\n");
    // Device re-enumerated - handle is no longer valid
    // For now, return true and let the caller know it "worked" but may need
    // reinit
    return true;
  } else {
