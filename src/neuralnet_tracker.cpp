...
int find_input_intensity_quantile(const cv::Mat &frame, float percentage) {
  ...
  cv::Mat hist;
  cv::calcHist(&frame, 1, channels, cv::Mat(), hist, 1, hist_size, ranges,
               true, false);

  int gray_level = 0;
  const int num_pixels_quantile =
      static_cast<int>(frame.total() * percentage * 0.01f);
...
Localizer::Localizer(Ort::MemoryInfo &allocator_info, Ort::Session &&session)
  ...
  input_val_ = Ort::Value::CreateTensor<float>(
      allocator_info, input_mat_.ptr<float>(0), input_mat_.total(), input_shape,
      4);

  const std::int64_t output_shape[2] = {1, 5};
  output_val_ = Ort::Value::CreateTensor<float>(
      allocator_info, results_.data(), results_.size(), output_shape, 2);
}
...
std::pair<float, cv::Rect2f> Localizer::run(const cv::Mat &frame) {
  ...
  session_.Run(Ort::RunOptions{nullptr}, input_names, &input_val_, 1,
  ...
  const cv::Rect2f roi = unnormalize_rect(
      cv::Rect2f{results_[1], results_[2], results_[3] - results_[1],
...
PoseEstimator::PoseEstimator(Ort::MemoryInfo &allocator_info,
                             Ort::Session &&session)
...

  {
    ...
    input_val_.push_back(Ort::Value::CreateTensor<float>(
        allocator_info, input_mat_.ptr<float>(0), input_mat_.total(), shape,
        4));
  }
  ...
  for (size_t i = 0; i < session_.GetInputCount(); ++i) {
    ...
    input_names_.push_back(
        std::string(&*session_.GetInputNameAllocated(i, allocator_)));
#else
...
std::optional<PoseEstimator::Face> PoseEstimator::run(const cv::Mat &frame,
                                                      const cv::Rect &box) {
  const int patch_size =
      static_cast<int>(std::max(box.width, box.height) * 1.05f);
  const cv::Point2f patch_center = {
      std::clamp<float>(box.x + 0.5f * box.width, 0.f,
                        static_cast<float>(frame.cols)),
      std::clamp<float>(box.y + 0.5f * box.height, 0.f,
  ...
  cv::resize(cropped, scaled_frame_, scaled_frame_.size(), 0, 0,
             cv::INTER_AREA);
  normalize_brightness(scaled_frame_, input_mat_);

  auto t_start = std::chrono::steady_clock::now();
  try {
    session_.Run(Ort::RunOptions{nullptr}, input_c_names_.data(),
                 input_val_.data(), input_val_.size(), output_c_names_.data(),
  ...
  const cv::Point2f center =
      patch_center + (0.5f * patch_size) *
                         cv::Point2f{output_coord_[0], output_coord_[1]};
  const float size = patch_size * 0.5f * output_coord_[2];

  cv::Quatf rotation = {output_quat_[3], output_quat_[0], output_quat_[1],
                        output_quat_[2]};
...
bool NeuralNetTracker::init(const std::string &localizer_path,
                            const std::string &posenet_path) {
  try {
    env_ = Ort::Env{OrtLoggingLevel::ORT_LOGGING_LEVEL_WARNING,
                    "ltr_neuralnet"};
    ...
    localizer_ = std::make_unique<Localizer>(
        alloc_info_, Ort::Session{env_, localizer_path.c_str(), opts});

    ltr_int_log_message("Loading posenet model: %s\n", posenet_path.c_str());
    posenet_ = std::make_unique<PoseEstimator>(
        alloc_info_, Ort::Session{env_, posenet_path.c_str(), opts});
...
void NeuralNetTracker::set_fov(float diagonal_fov_degrees) {
  const float diag_fov =
      diagonal_fov_degrees * static_cast<float>(M_PI) / 180.0f;
  const float fov_w =
      2.0f * std::atan(std::tan(diag_fov / 2.0f) /
                       std::sqrt(1.0f + 0.75f * 0.75f));
  const float fov_h =
      2.0f * std::atan(std::tan(diag_fov / 2.0f) /
                       std::sqrt(1.0f + 4.0f / 3.0f * 4.0f / 3.0f));
...
void NeuralNetTracker::quaternion_to_euler(const cv::Quatf &q, float &pitch,
...
void NeuralNetTracker::image_to_world_position(const cv::Point2f &center,
                                               float face_size,
  ...
  const float zpos =
      (center.x / image_size.width * 2.0f - 1.0f) * xpos / focal_length_w_;
  const float ypos =
...
bool NeuralNetTracker::detect(const cv::Mat &frame, float &pitch, float &yaw,
                              float &roll, float &tx, float &ty, float &tz) {
  ...
  if (!last_localizer_roi_ || !last_roi_ ||
      iou(*last_localizer_roi_, *last_roi_) < 0.25f) {
...
void NeuralNetTracker::draw_gizmos(cv::Mat &frame) const {
  ...
  char text[128];
  snprintf(text, sizeof(text), "P:%.1f Y:%.1f R:%.1f", last_pitch_, last_yaw_,
           last_roll_);
  cv::putText(frame, text, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 0.7,
              cv::Scalar(0, 255, 0), 2);

  snprintf(text, sizeof(text), "Tx:%.1f Ty:%.1f Tz:%.1f", last_tx_, last_ty_,
           last_tz_);
