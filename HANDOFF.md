...
- **DCS UDP startup hardening (fix):** [src/ltr_udp.c](src/ltr_udp.c) previously exited if `linuxtrack_get_tracking_state()` did not become `RUNNING` or `PAUSED` within roughly ten seconds. On a fresh config/profile this left the Wine NPClient DLL fully loaded and encrypted (`Black Shark`, `encrypted=1`) but with no incoming UDP pose datagrams, so DCS saw TrackIR briefly and then dropped it. `ltr_udp` now stays alive and sends neutral frames while Linuxtrack is still initializing, then switches to real pose data when the tracker reaches `RUNNING`/`PAUSED`.

- **No-space `Prefix="..."` parser mismatch (fix):** [src/linuxtrack.c](src/linuxtrack.c) already accepted both spaced and no-space `Prefix` assignments, but [src/utils.c](src/utils.c) only accepted `Prefix = "..."`. Fresh AppImage configs can contain `Prefix="/tmp/.mount_.../usr/bin"`, so `ltr_udp` could load `liblinuxtrack` while the lower server launcher failed to resolve `ltr_server1`, logging `Couldn't find prefix!` and `execv: Bad address`. `ltr_int_get_app_path()` now accepts the same assignment forms as the public loader, and [src/ltlib.c](src/ltlib.c) now fails cleanly if `ltr_server1` cannot be resolved.

- **Wine hotkey routing cleanup (fix):** [src/wine_bridge/client/ltr_wine_hotkeys.c](src/wine_bridge/client/ltr_wine_hotkeys.c) now sends `RECN`/`PAUS` directly to the Wine NPClient bridge on UDP port `4242` instead of routing through the host GUI listener on `4243`. The old path depended on the UDP settings dialog having an active `commandReceived` connection, so hotkeys could stop working after closing the dialog. [src/wine_bridge/client/linuxtrack_udp.c](src/wine_bridge/client/linuxtrack_udp.c) now handles those commands in the in-game bridge receiver: pause toggles local bridge output and Wine hotkey recenter calls the bridge's `linuxtrack_recenter()` path. Host-side GUI recenter uses a separate `RSET` bridge command to clear DLL offsets after the host tracker has already recentered, avoiding a double-recenter.

- **Wine game profile notification (fix):** [src/wine_bridge/client/NPClient_udp_main.c](src/wine_bridge/client/NPClient_udp_main.c) resolves the NaturalPoint profile ID (`1003` -> `Black Shark`, etc.) from `gamedata.txt`, initializes the Wine bridge under that name, and now calls [src/wine_bridge/client/linuxtrack_udp.c](src/wine_bridge/client/linuxtrack_udp.c) to notify the Linux GUI via `PROF<name>` on UDP port `4243`. [src/qt_gui/udp_bridge.cpp](src/qt_gui/udp_bridge.cpp) now emits the full datagram instead of truncating to four bytes, and [src/qt_gui/ltr_gui.cpp](src/qt_gui/ltr_gui.cpp) owns the persistent handler that applies `udpBridge->setProfile(profile)`. This lets `ltr_udp` restart under the game profile and trigger Linuxtrack's existing missing-profile creation path instead of remaining on `Default`.

- **TrackIR runtime asset path clarification:** The files used by the runtime and Wine bridge are under `~/.config/linuxtrack/tir_firmware`, not the generic LAL asset store under `~/.local/share/linuxtrack/lal/...`. For TrackIR 5.5.3, the supported clean-install path is the legacy **Reinstall TrackIR firmware** flow with the downloaded `TrackIR_5.5.3.exe`; that path runs the installer through Wine with its license UI visible and then extracts the installed payloads.

- **LAL status for TrackIR:** Treat LAL's TrackIR row as informational/experimental for now. The active, verified TrackIR runtime path is **Misc → Reinstall TrackIR firmware** writing to `~/.config/linuxtrack/tir_firmware`. LAL's generic asset-store directory is not consumed by the current TrackIR runtime or Wine bridge, so future work should either hide/disable TrackIR in LAL or make LAL delegate to the same legacy extractor/output path.

- **Installer bundle limitation:** TrackIR 5.5.3's downloaded `TrackIR_5.5.3.exe` is a WiX Burn bootstrapper. Plain `7z e` only exposes bootstrapper payload files (`u0`, `u1`, etc.), not the final installed `TrackIR5.exe`/`sgl.dat` pair. Use the legacy Wine install path for direct installer handling; it runs the vendor installer first, then scans the installed payloads for firmware and game data.
...
- **End-to-end clean AppImage verification:** Deleted `~/.config/linuxtrack`, launched the new AppImage, used **Misc → Reinstall TrackIR firmware** with the downloaded `TrackIR_5.5.3.exe`, then verified DCS World, Elite Dangerous, and X4 Foundations tracking. This confirms the clean-install path is `~/.config/linuxtrack/tir_firmware` plus the Wine/Proton UDP bridge, not LAL's generic asset store.
...
- **UDP bridge log path fix (cleanup):** `module_dir_path()` in [src/wine_bridge/client/linuxtrack_udp.c](src/wine_bridge/client/linuxtrack_udp.c) called `GetModuleFileNameA` to derive the log directory, which returned a Win32 path like `C:\Program Files\Linuxtrack\`. The DLL's `fopen()` is GLIBC's, not msvcrt's — it does not translate `C:\...` paths and instead created literal files named `C:\Program Files\Linuxtrack\ltr_udp_client.log` (with backslashes) inside the game's install directory. Replaced with a POSIX-side path resolver that writes to `$XDG_CONFIG_HOME/linuxtrack/wine_bridge.log` (or `$HOME/.config/linuxtrack/wine_bridge.log`), matching where the rest of linuxtrack stores its config and state. Removed the `fopen("C:\\...")` and bare-cwd fallbacks — they only produced garbage filenames. Single shared log across all Wine prefixes; `udp_log` is `extern` from `NPClient_udp_main.c` so the same fix covers both translation units.
...
- **macOS Bring-up Phase 2 (2026-03-12, started):** Modernized the experimental mac app-shell target in [src/qt_gui/CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/CMakeLists.txt) so `ltr_gui` becomes a real `MACOSX_BUNDLE` only when `APPLE AND BUILD_MAC_EXPERIMENTAL`. Added [src/qt_gui/Info.plist.in](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/Info.plist.in) for bundle metadata, versioning, icon wiring, and camera-permission text; reused the existing [src/qt_gui/linuxtrack.icns](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/linuxtrack.icns) asset; and updated installation so experimental mac builds produce `Linuxtrack.app` without changing Linux packaging semantics.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Made the experimental app shell self-contained enough for a future launch smoke test. [src/qt_gui/CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/CMakeLists.txt) now embeds `linuxtrack1.conf`, `sources_mac.txt`, the existing GUI data assets, and Qt help files into `Linuxtrack.app/Contents/Resources/linuxtrack`. [src/qt_gui/ltr_gui_prefs.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/ltr_gui_prefs.cpp) now resolves bundle-local resources before falling back to Linux/AppImage-style paths, and [src/qt_gui/main.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/main.cpp) now sets stable Qt organization/application metadata so settings persistence has a defined identity on macOS. Verified again from Linux with `cmake -S . -B build_phase0_verify`, `cmake --build build_phase0_verify --target ltr_gui -j"$(nproc)"`, and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Isolated more Linux-only build graph assumptions from the experimental mac path. [CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/CMakeLists.txt) now force-disables Mickey and the Wine bridge when `APPLE AND BUILD_MAC_EXPERIMENTAL`, skips Linux desktop/icon/AppStream installs on macOS, and only requests X11 for Mickey on non-Apple Unix. [src/CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/src/CMakeLists.txt) now keeps `ltr_hotkeyd`, `ltr_hotkey_gui`, `scs_plugin`, and `npclient_linux` out of the mac target graph. Re-verified the Linux side with `cmake -S . -B build_phase0_verify`, `cmake --build build_phase0_verify --target ltr_gui -j"$(nproc)"`, and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Added [scripts/configure_macos_experimental.sh](/home/startux/Code/linuxtrackfixed/linuxtrack/scripts/configure_macos_experimental.sh) as the explicit configure entry point for future mac testers. The script turns on `BUILD_MAC_EXPERIMENTAL`, disables Mickey, the Wine bridge, and the neural tracker for this app-shell phase, and stages installs under a repo-local prefix instead of Linux’s `/opt/linuxtrack`. This gives Phase 2 a documented, reproducible configure/build/install path without changing the default Linux workflow.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Reduced more runtime `/opt/linuxtrack` assumptions inside the Qt GUI. [src/qt_gui/ltr_gui_prefs.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/ltr_gui_prefs.h) and [src/qt_gui/ltr_gui_prefs.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/ltr_gui_prefs.cpp) now expose shared runtime discovery helpers, and [src/qt_gui/udp_bridge.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/udp_bridge.cpp), [src/qt_gui/ltr_gui.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/ltr_gui.cpp), [src/qt_gui/udp_settings.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/udp_settings.cpp), [src/qt_gui/plugin_install.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/plugin_install.cpp), and [src/qt_gui/xplugin.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/xplugin.cpp) now use those helpers for executable/plugin/registry discovery instead of assuming a fixed Linux install prefix. Re-verified with `cmake --build build_phase0_verify --target ltr_gui -j"$(nproc)"` and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Finished the last obvious GUI-side fixed-prefix cleanup by switching the remaining hotkey lookup in [src/qt_gui/udp_settings.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/udp_settings.cpp) to the shared runtime helper as well. Also improved the public client loader in [src/linuxtrack.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/linuxtrack.c) so Darwin builds try `@executable_path/../Frameworks/liblinuxtrack.dylib`, `@loader_path/../Frameworks/liblinuxtrack.dylib`, and `@rpath/liblinuxtrack.dylib` before falling back to the legacy config-prefix search. Re-verified with `cmake --build build_phase0_verify --target linuxtrack ltr_gui -j"$(nproc)"` and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Hardened the top-level install defaults for the experimental mac path in [CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/CMakeLists.txt). `CMAKE_INSTALL_PREFIX` now defaults to a repo-local staging directory instead of `/opt/linuxtrack` when `APPLE AND BUILD_MAC_EXPERIMENTAL`, and install RPATHs now use `@loader_path`-based mac bundle/library locations while preserving the existing Linux `/opt` and `$ORIGIN` behavior. Re-verified with `cmake -S . -B build_phase0_verify`, `cmake --build build_phase0_verify --target linuxtrack ltr_gui -j"$(nproc)"`, and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Aligned the generated data-path configuration with the experimental mac bundle layout in [CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/CMakeLists.txt). When `APPLE AND BUILD_MAC_EXPERIMENTAL`, `Linuxtrack_DATA_DIR` now points at `Linuxtrack.app/Contents/Resources/linuxtrack` instead of `share/linuxtrack`, which means `LTR_DATA_PATH` and generated [src/pathconfig.h.cmake](/home/startux/Code/linuxtrackfixed/linuxtrack/src/pathconfig.h.cmake) now describe the same bundle resource tree that Phase 2 actually installs. Re-verified with `cmake -S . -B build_phase0_verify`, `cmake --build build_phase0_verify --target linuxtrack ltr_gui -j"$(nproc)"`, and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Added a first-run bundle fallback to low-level path resolution in [src/utils.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/utils.c). On Darwin, `ltr_int_get_app_path()` now uses `_NSGetExecutablePath` and `realpath()` to derive paths relative to the current executable when the normal config-prefix lookup is missing or incomplete, which means bundle-local data/library discovery is no longer blocked on `~/.config/linuxtrack/linuxtrack1.conf` existing first. Re-verified with `cmake --build build_phase0_verify --target ltr linuxtrack ltr_gui -j"$(nproc)"` and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Standardized helper-binary lookup in the shared utilities layer. [src/utils.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/utils.h) and [src/utils.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/utils.c) now expose `ltr_int_get_helper_path()`, which prefers `Contents/Resources/linuxtrack/helper` on Darwin and falls back to the old `../helper` layout. [src/macwebcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c) and [src/qt_gui/macwebcam_info.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/macwebcam_info.cpp) now use that helper abstraction instead of baking in `../helper/qt_cam`. Re-verified with `cmake --build build_phase0_verify --target ltr linuxtrack ltr_gui -j"$(nproc)"` and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Extended the same helper-path cleanup into the Qt extractor UI. [src/qt_gui/ltr_gui_prefs.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/ltr_gui_prefs.h) and [src/qt_gui/ltr_gui_prefs.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/ltr_gui_prefs.cpp) now expose `PrefProxy::getHelperPath()`, and [src/qt_gui/extractor.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/qt_gui/extractor.cpp) now uses it for `cabextract` instead of climbing out of the data directory with `../../helper`. Re-verified with `cmake --build build_phase0_verify --target ltr_gui -j"$(nproc)"` and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Removed one more legacy temp-path assumption from the old mac capture code. [src/macwebcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c) no longer uses the fixed `/tmp/xxx` mmap filename for `qt_cam`; it now requests `ltr_int_get_ipc_path("macwebcam_capture.mmap")`, which keeps the helper’s shared state under linuxtrack-managed runtime locations instead of a global temp file name. Re-verified with `cmake --build build_phase0_verify --target ltr linuxtrack ltr_gui -j"$(nproc)"` and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Reduced first-run dependence on a pre-existing user config in the shared prefs layer. [src/pref.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/pref.cpp) now falls back to the packaged `linuxtrack1.conf` from `ltr_int_get_data_path()` when `~/.config/linuxtrack/linuxtrack1.conf` is missing, which gives the experimental app shell and other non-GUI startup paths a sane packaged baseline before the user config exists. Re-verified with `cmake --build build_phase0_verify --target ltr linuxtrack ltr_gui -j"$(nproc)"` and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Updated the non-GUI startup diagnostics in [src/ltr_server1.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/ltr_server1.c) so path-validation errors now describe the current install/bundle lookup model instead of assuming the user config is always missing or wrong. Re-verified with `cmake --build build_phase0_verify --target ltr ltr_server1 -j"$(nproc)"` and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Reduced another Linux-style home-layout assumption in the shared startup path. [src/utils.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/utils.c) now prefers `XDG_CONFIG_HOME/linuxtrack/...` before `HOME/.config/linuxtrack/...` for default config and runtime-state paths, and [src/linuxtrack.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/linuxtrack.c) now does the same when the public client loader reads the saved prefix. Re-verified with `cmake --build build_phase0_verify --target ltr linuxtrack ltr_server1 ltr_gui -j"$(nproc)"` and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Removed the last machine-specific fallback paths from the Wine bridge UDP client in [src/wine_bridge/client/rest.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/wine_bridge/client/rest.c). The bridge still uses its existing `HOME`, `USERPROFILE`, and current-directory fallback chain for firmware metadata lookup, but it no longer silently probes `/home/startux/.config/linuxtrack/...`. Re-verified with `cmake --build build_phase0_verify --target NPClientUDP64 -j"$(nproc)"` and `git diff --check`.
- **macOS Bring-up Phase 2 (2026-03-12, continued):** Normalized config-base lookup inside the Wine bridge client helpers. [src/wine_bridge/client/rest.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/wine_bridge/client/rest.c) and [src/wine_bridge/client/check_data.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/wine_bridge/client/check_data.c) now derive their `linuxtrack` config base from `XDG_CONFIG_HOME`, then `HOME`, then `USERPROFILE`, instead of hardcoding `HOME/.config/linuxtrack`. Re-verified with `cmake --build build_phase0_verify --target NPClientUDP64 -j"$(nproc)"`; `CheckData` is not enabled in this build because the 32-bit Wine path is unavailable here.
- **macOS Bring-up Phase 2 (2026-03-12, completed repository-side):** Added [scripts/verify_macos_bundle_layout.sh](/home/startux/Code/linuxtrackfixed/linuxtrack/scripts/verify_macos_bundle_layout.sh) as the final bundle-content verification step for the experimental app shell. The script validates that an installed `Linuxtrack.app` contains the expected executable, `Info.plist`, icon, bundled default config, selected runtime resources, and Qt help files. Verified the script with `bash -n scripts/verify_macos_bundle_layout.sh` and against a synthetic `Linuxtrack.app` tree in `/tmp`. Phase 2 is now complete on the repository side; what remains is external real-mac launch/persistence validation once a tester machine exists.
- **macOS Bring-up Phase 3 (2026-03-12, started):** Landed the first shared capture seam before touching modern mac camera APIs. Added [src/frame_adapter.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/frame_adapter.h) and [src/frame_adapter.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/frame_adapter.c) so raw camera buffers can be converted into the existing grayscale tracking input path through a platform-neutral helper instead of private V4L2-only code. [src/webcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/webcam_driver.c) now uses that helper, and [src/tests/test_frame_adapter.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/test_frame_adapter.cpp) plus [src/tests/CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/CMakeLists.txt) add regression coverage for `YUYV`, `GREY`, `RGB3`, `BGR3`, and unsupported-format fallback behavior. Verified with `cmake -S . -B build_phase0_verify`, `cmake --build build_phase0_verify --target test_frame_adapter test_tracking ltr_webcam_probe wc -j"$(nproc)"`, direct runs of `./build_phase0_verify/src/tests/test_frame_adapter` and `./build_phase0_verify/src/tests/test_tracking`, and `git diff --check`. During verification I also cleared one stale zero-byte `test_tracking.cpp.o` artifact left behind in the build tree by the earlier interrupted session.
- **macOS Bring-up Phase 3 (2026-03-12, continued):** Extracted shared frame-clock/finalization policy into [src/frame_adapter.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/frame_adapter.h) and [src/frame_adapter.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/frame_adapter.c). The new helpers let capture paths finalize `frame.counter` and `frame.usec` consistently before entering the tracking pipeline, including no-frame/drop handling, duplicate timestamp clamping, backward timestamp clamping, and wrap-sized jumps. [src/runloop.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/runloop.c) now uses that shared path instead of assigning counter/timestamp inline, and [src/tests/test_frame_adapter.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/test_frame_adapter.cpp) now covers the new frame-clock semantics. Re-verified with `cmake --build build_phase0_verify --target test_frame_adapter test_tracking ltr_webcam_probe wc -j"$(nproc)"`, direct runs of `./build_phase0_verify/src/tests/test_frame_adapter` and `./build_phase0_verify/src/tests/test_tracking`, and `git diff --check`.
- **macOS Bring-up Phase 3 (2026-03-12, continued):** Extracted another shared capture handoff seam into [src/frame_adapter.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/frame_adapter.h) and [src/frame_adapter.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/frame_adapter.c). The adapter layer now owns common frame setup (`width`, `height`, expected blob count, cleared diagnostics), preview/tracking buffer selection, and grayscale thresholding, so capture backends can hand off into blob/face processing without reimplementing those mechanics. [src/webcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/webcam_driver.c) now uses those helpers, and [src/tests/test_frame_adapter.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/test_frame_adapter.cpp) covers the new setup/buffer/threshold semantics. Re-verified with `cmake --build build_phase0_verify --target test_frame_adapter test_tracking ltr_webcam_probe wc -j"$(nproc)"`, direct runs of `./build_phase0_verify/src/tests/test_frame_adapter` and `./build_phase0_verify/src/tests/test_tracking`, and `git diff --check`.
- **macOS Bring-up Phase 3 (2026-03-12, continued):** Extracted the non-face shared grayscale-to-blob path into [src/capture_process.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/capture_process.h) and [src/capture_process.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/capture_process.c). The new `ltr_int_extract_blobs_from_gray_image()` helper owns stripes/blob extraction plus optional axis flipping, so camera backends can reuse one blob-processing entry point instead of keeping that logic inside each driver. [src/webcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/webcam_driver.c) and [src/ps3eye_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/ps3eye_driver.c) now use it for non-OpenCV capture, [src/CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/src/CMakeLists.txt) links it into `ltr`, and [src/tests/test_image_process.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/test_image_process.cpp) plus [src/tests/CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/CMakeLists.txt) cover the shared helper’s threshold-preserving and flip behavior. Re-verified from a fresh build tree with `cmake -S . -B build_phase3_verify`, `cmake --build build_phase3_verify --target test_image_process test_frame_adapter test_tracking ltr_webcam_probe wc -j"$(nproc)"`, direct runs of `./build_phase3_verify/src/tests/test_image_process`, `./build_phase3_verify/src/tests/test_frame_adapter`, and `./build_phase3_verify/src/tests/test_tracking`, and `git diff --check`.
- **macOS Bring-up Phase 3 (2026-03-12, continued):** Extracted the gray-frame processing route itself into [src/capture_process.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/capture_process.h) and [src/capture_process.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/capture_process.c). The new `ltr_int_process_gray_image()` helper routes gray frames through either shared blob extraction or face detection, which removes that branch from [src/webcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/webcam_driver.c) and gives future mac capture work one processing entry point to target. [src/tests/test_capture_process.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/test_capture_process.cpp) plus [src/tests/CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/CMakeLists.txt) add stub-based regression coverage for both blob and face routes. Re-verified from `build_phase3_verify` with `cmake --build build_phase3_verify --target test_capture_process test_image_process test_frame_adapter test_tracking ltr_webcam_probe wc -j"$(nproc)"`, direct runs of `./build_phase3_verify/src/tests/test_capture_process`, `./build_phase3_verify/src/tests/test_image_process`, `./build_phase3_verify/src/tests/test_frame_adapter`, and `./build_phase3_verify/src/tests/test_tracking`, and `git diff --check`.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Added a mac-facing gray-frame replay/provider seam in [src/capture_replay.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/capture_replay.h) and [src/capture_replay.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/capture_replay.c). The new `ltr_int_replay_gray_capture_frame()` helper takes an already-grayscale frame, applies shared frame setup, chooses preview/processing buffers, thresholds non-face frames, preserves face-tracking frames, and then routes the result through the shared capture-processing path. This is the first explicit entry point intended for future AVCapture-style providers or recorded mac frame replay without going through the old `qt_cam` + mmap path. [src/tests/test_capture_replay.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/test_capture_replay.cpp) plus [src/tests/CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/CMakeLists.txt) add regression coverage for thresholded non-face replay and passthrough face replay. Re-verified from `build_phase3_verify` with `cmake -S . -B build_phase3_verify`, `cmake --build build_phase3_verify --target test_capture_replay test_capture_process test_image_process test_frame_adapter test_tracking ltr_webcam_probe wc -j"$(nproc)"`, direct runs of `./build_phase3_verify/src/tests/test_capture_replay`, `./build_phase3_verify/src/tests/test_capture_process`, `./build_phase3_verify/src/tests/test_image_process`, `./build_phase3_verify/src/tests/test_frame_adapter`, and `./build_phase3_verify/src/tests/test_tracking`, and `git diff --check`.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Added the first explicit grayscale provider contract in [src/capture_provider.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/capture_provider.h) and [src/capture_provider.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/capture_provider.c). `ltr_gray_capture_provider` plus `ltr_int_provider_get_frame()` define a minimal callback/vtable seam around the replay helper: providers return `1` for a new frame, `0` for no frame, or `< 0` for error, and the shared code handles replay plus `frame_acquired` propagation consistently. [src/tests/test_capture_provider.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/test_capture_provider.cpp) and [src/tests/CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/CMakeLists.txt) add regression coverage for acquired-frame forwarding, no-frame propagation, and replay-failure propagation. Re-verified from `build_phase3_verify` with `cmake --build build_phase3_verify --target test_capture_provider test_capture_replay test_capture_process test_image_process test_frame_adapter test_tracking ltr_webcam_probe wc -j"$(nproc)"`, direct runs of `./build_phase3_verify/src/tests/test_capture_provider`, `./build_phase3_verify/src/tests/test_capture_replay`, `./build_phase3_verify/src/tests/test_capture_process`, `./build_phase3_verify/src/tests/test_image_process`, `./build_phase3_verify/src/tests/test_frame_adapter`, and `./build_phase3_verify/src/tests/test_tracking`, and `git diff --check`.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Added a tiny mac-oriented provider scaffold in [src/mac_capture_stub.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/mac_capture_stub.h) and [src/mac_capture_stub.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/mac_capture_stub.c). `ltr_mac_capture_stub` is an in-memory implementation of the new grayscale provider contract that can queue one synthetic or recorded grayscale frame at a time, making it a concrete landing point for future experimental mac capture work before real camera APIs are attached. [src/tests/test_mac_capture_stub.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/test_mac_capture_stub.cpp) plus [src/tests/CMakeLists.txt](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/CMakeLists.txt) add regression coverage for stub initialization, one-shot frame delivery, and face-tracking frame submission. Re-verified from `build_phase3_verify` with `cmake -S . -B build_phase3_verify`, `cmake --build build_phase3_verify --target test_mac_capture_stub test_capture_provider test_capture_replay test_capture_process test_image_process test_frame_adapter test_tracking ltr_webcam_probe wc -j"$(nproc)"`, direct runs of `./build_phase3_verify/src/tests/test_mac_capture_stub`, `./build_phase3_verify/src/tests/test_capture_provider`, `./build_phase3_verify/src/tests/test_capture_replay`, `./build_phase3_verify/src/tests/test_capture_process`, `./build_phase3_verify/src/tests/test_image_process`, `./build_phase3_verify/src/tests/test_frame_adapter`, and `./build_phase3_verify/src/tests/test_tracking`, and `git diff --check`.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Started the first direct legacy-mac integration onto the shared replay seam in [src/macwebcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c). When the old `qt_cam` helper publishes a grayscale frame and the caller requested a preview/processed buffer, the driver now packages that frame through `ltr_int_replay_gray_capture_frame()` to populate shared preview/processing buffers instead of only hand-copying the raw bitmap. The helper-provided blobs remain the source of truth for pose input for now; this step only moves preview/processing population closer to the new shared pipeline. This change has not been compiled on a real mac build in this Linux environment yet.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Tightened the shared replay seam and pushed the legacy mac helper path one step closer to the provider contract. [src/capture_replay.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/capture_replay.c) now tolerates single-buffer callers by reusing whichever preview/processing buffer is available, and [src/tests/test_capture_replay.cpp](/home/startux/Code/linuxtrackfixed/linuxtrack/src/tests/test_capture_replay.cpp) covers that case. [src/macwebcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c) now wraps the old `qt_cam` grayscale frame handoff in a local `ltr_gray_capture_provider` callback and feeds that through the shared provider/replay flow instead of constructing replay inputs inline. Linux-side shared verification passed again with `cmake --build build_phase3_verify --target test_mac_capture_stub test_capture_provider test_capture_replay test_capture_process test_image_process test_frame_adapter test_tracking ltr_webcam_probe wc -j"$(nproc)"`, direct runs of `./build_phase3_verify/src/tests/test_capture_replay` and `./build_phase3_verify/src/tests/test_capture_provider`, and `git diff --check`. The `macwebcam_driver.c` integration itself still has not been compiled on a real mac build in this Linux environment.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Reduced another chunk of remaining inline legacy-mac state handling in [src/macwebcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c). The driver now uses `ltr_int_prepare_capture_frame()` from [src/frame_adapter.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/frame_adapter.h) for width/height/blob-capacity setup and isolates helper-blob synchronization in a dedicated local helper instead of mixing those mechanics directly into `ltr_int_tracker_get_frame()`. This keeps the file moving toward a provider-backed adapter shape even though the helper-supplied blobs are still the source of truth for pose input. This mac-specific path still has not been compiled in a real mac build from this Linux environment.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Replaced the last ad hoc provider state in [src/macwebcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c) with the shared mac-facing scaffold from [src/mac_capture_stub.h](/home/startux/Code/linuxtrackfixed/linuxtrack/src/mac_capture_stub.h) and [src/mac_capture_stub.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/mac_capture_stub.c). The legacy `qt_cam` helper path now submits grayscale frames into `ltr_mac_capture_stub` and reads them back through the common provider contract, so future experimental mac capture work can target the same provider shape instead of maintaining a file-local vtable/context pair. Shared verification for the scaffold itself is being re-run on Linux; this mac-specific path still has not been compiled in a real mac build from this Linux environment.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Relaxed the legacy helper blob dependency in [src/macwebcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c). If the `qt_cam` helper publishes a grayscale frame that the shared provider/replay path can process but does not also publish a fresh helper-blob side-channel update, the driver now still reports `frame_acquired` and uses the shared grayscale-processing result already stored in `frame->bloblist`. Fresh helper blobs still override that fallback when available, preserving the current source-of-truth preference while making the old mac path behave more like the shared capture pipeline. This mac-specific path still has not been compiled in a real mac build from this Linux environment.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Narrowed the legacy helper blob side channel in [src/macwebcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c). For plain `mac_webcam`, a successfully processed shared grayscale frame now returns immediately and serves as the primary blob source; the helper blob path remains primarily for the legacy face-tracking category (`mac_webcam_ft`) and as a no-gray-frame fallback. This pushes the old mac webcam path closer to the same “gray frame in, shared processing out” shape already used by the Linux capture pipeline without dropping the remaining helper-specific face path yet.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Isolated the remaining legacy face-specific policy in [src/macwebcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c) behind small helpers (`is_face_tracking_device()`, `should_require_cascade()`, `should_accept_gray_frame_directly()`) instead of scattering raw `mac_webcam_ft` checks through initialization and frame acquisition. This does not change the current behavior split, but it makes the last helper-dependent face path explicit and bounded so the next replacement step can target one seam instead of rediscovering category policy across the file.
- **macOS Bring-up Phase 3 (2026-03-13, continued):** Promoted the shared grayscale-processing route to the primary acquisition path for both `mac_webcam` and `mac_webcam_ft` in [src/macwebcam_driver.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c). The legacy helper blob channel is now treated as an optional override layer: if helper blobs are available for a frame they replace the shared result, otherwise the already-processed shared `frame->bloblist` is used. This leaves the remaining legacy helper behavior as a bounded post-process seam instead of a delivery gate, which is the right shape for eventually deleting or replacing it.

**Phase 3 current state summary (2026-03-14):**
- [`src/CMakeLists.txt`](/home/startux/Code/linuxtrackfixed/linuxtrack/src/CMakeLists.txt) now links [src/mac_capture_stub.c](/home/startux/Code/linuxtrackfixed/linuxtrack/src/mac_capture_stub.c) into the core `ltr` library, so the mac-facing provider scaffold is no longer test-only.
- [`src/macwebcam_driver.c`](/home/startux/Code/linuxtrackfixed/linuxtrack/src/macwebcam_driver.c) now treats the shared grayscale provider/replay/process path as the primary frame path for both `mac_webcam` and `mac_webcam_ft`.
- The old helper blob side channel is now optional and only acts as a post-process override when fresh helper blobs are present for a frame.
- The main remaining legacy dependency in this file is the `qt_cam` helper itself as the gray-frame source, plus face-mode cascade setup during initialization.
- This repository-side progress has shared Linux verification for the provider/replay/process seams, but the mac-specific `libmacwc` path still has not been compiled or run on a real macOS build in this environment.
